  ...
  return {
    init: function(page, callback) {
      if(initialized) {
       ...
       $(taskPage).find('#tblTasks').on('click', 'tbody td:not(:last-child)', function(evt) {
     		$(evt.target).toggleClass('rowHighlight');
         $(evt.target).siblings('td:not(:last-child)').toggleClass('rowHighlight');
     	});
       ...
       $(taskPage).find('#tblTasks tbody').on('click', '.deleteRow', function(evt) {
         ...
         storageEngine.delete('task', $(evt.target).data().taskId,
          function() {
            $(evt.target).parents('tr').remove(); taskCountChanged();
          }, errorLogger);
        });
  ...
  loadTasks : function() {
    storageEngine.findAll('task', function(tasks) {
      ...
      $.each(tasks, function(index, task) {
        ...
        var template = $.templates("#taskRow");
        var htmlOutput = template.render(task);
        $(taskPage).find('#tblTasks tbody').append(htmlOutput);
